boxplot(FBSCurrent.Comb$BeforeAIP~FBSCurrent.Comb$BeforeAIP.Cat)

?predict.glm
FBSCurrent<-read.csv("FBScurrent.csv")
...
FBSCurrent$BeforeOther1<-(FBSCurrent$BeforeCrisis+FBSCurrent$BeforeCRR+FBSCurrent$BeforeHH+FBSCurrent$BeforeOther+ FBSCurrent$BeforePreschool+ FBSCurrent$BeforeResidential)

FBSCurrent$DuringOther1<-(FBSCurrent$DuringCrisis+FBSCurrent$DuringCRR+FBSCurrent$DuringHH+FBSCurrent$DuringOther+ FBSCurrent$DuringPreschool+ FBSCurrent$DuringResidential)

FBSCurrent$AfterOther1<-(FBSCurrent$AfterCrisis+FBSCurrent$AfterOther+ FBSCurrent$AfterPreschool) #Did not include CRR, Host Home, and Residentialin the after category as they are outcomes
...
FBSCurrent.Comb<-FBSCurrent[AllTestVariables] 
...
FBSCurrent.Comb$BeforeAIP.Cat <- as.factor(ifelse(FBSCurrent.Comb$BeforeAIP == 0, 0, ifelse(FBSCurrent.Comb$BeforeAIP <=30, 1, 2)))
...
predicted.FBSCurrent.ModelBefore2 <- predict(FBSComb.LRmodelBefore2,newdata=FBSCurrent.Comb,type='response') #predicts odds of being removed from community

#checking mean predicted value for those removed and those not removed
mean(predicted.FBSCurrent.ModelBefore2[FBSCurrent.Comb$FinalRiskincDHS == "In Community"])
mean(predicted.FBSCurrent.ModelBefore2[FBSCurrent.Comb$FinalRiskincDHS == "Removed From Community"])

#adding probability column to FBS.Comb, exploring outcomes
FBSCurrent.Comb$ProbabilityBefore <- predicted.FBSCurrent.ModelBefore2
summary(FBSCurrent.Comb)
boxplot(FBSCurrent.Comb$ProbabilityBefore ~ FBSCurrent.Comb$FinalRiskincDHS, main = "Predicted probability vs reality")
boxplot.stats(FBSCurrent.Comb$ProbabilityBefore[FBSCurrent.Comb$FinalRiskincDHS== "In Community"])
boxplot.stats(FBSCurrent.Comb$ProbabilityBefore[FBSCurrent.Comb$FinalRiskincDHS== "Removed From Community"])

#Predicting Risk of Removal
predicted.FBSCurrent.ModelAfter <- predict(FBSComb.LRmodelAfter,newdata=FBSCurrent.Comb,type='response') #predicts odds of being removed from community
#checking mean predicted value for those removed and those not removed
mean(predicted.FBSCurrent.ModelAfter[FBSCurrent.Comb$FinalRiskincDHS == "In Community"])
mean(predicted.FBSCurrent.ModelAfter[FBSCurrent.Comb$FinalRiskincDHS == "Removed From Community"])

#adding probability column
FBSCurrent.Comb$ProbabilityAfter <- predicted.FBSCurrent.ModelAfter
summary(FBSCurrent.Comb)
boxplot(FBSCurrent.Comb$ProbabilityAfter ~ FBSCurrent.Comb$FinalRiskincDHS, main = "Predicted probability vs reality")
boxplot.stats(FBSCurrent.Comb$ProbabilityAfter[FBSCurrent.Comb$FinalRiskincDHS== "In Community"])
boxplot.stats(FBSCurrent.Comb$ProbabilityAfter[FBSCurrent.Comb$FinalRiskincDHS== "Removed From Community"])
